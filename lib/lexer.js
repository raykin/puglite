...
Lexer.prototype = {
   ...
   * Text.
   */
  addText: function(type, value) {
    var tok = this.tok(type, value);
  ...
  /**
   * Reject stripped-feature keywords with one uniform error.
   */

  unsupported: function() {
    var captures = /^(each|for|while|case|when|default|if|unless|else|mixin|extends|include|block|append|prepend|yield)\b/.exec(
      this.input
    );
    var feature = captures && captures[1];
    if (!feature && /^\+[\s(]*[-\w#]/.test(this.input)) feature = 'mixin call (+)';
    if (!feature && /^:[\w-]/.test(this.input)) feature = 'filter (:)';
    if (feature) {
      this.error(
        'UNSUPPORTED_SYNTAX',
        '`' +
          feature +
          '` is not supported in puglite, which only transforms markup. Use your framework for logic, iteration, and data binding.'
      );
    }
  },
  ...
  advance: function() {
    return (
      ...
      this.callLexerFunction('unsupported') ||
