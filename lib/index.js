...
function compileBody(str, options) {
  ...
  // Lex
  var tokens = lex(str, { filename: options.filename });

  // Parse
  tokens = stripComments(tokens, { filename: options.filename });
  var ast = parse(tokens, { filename: options.filename, src: str });
  ...
  // dependencies is always empty: include/extends were stripped from puglite;
  // the field remains for API compatibility with pug
 ...
 *   - `compileDebug` when `false` debugging code is stripped from the compiled
       ...
       the compiled template for better accuracy. Defaults to `true`.
 *   - `inlineRuntimeFunctions` defaults to `false`: the template references the
 *     shared runtime instead of inlining it. Note this default differs from
 *     `compileClient*`, which defaults to `true`.
 ...
 *     the compiled template for better error messages. Defaults to `false`
 *     (unlike `compile()`, which defaults to `true`) to keep client bundles lean.
 *   - `inlineRuntimeFunctions` defaults to `true` so the output is
 *     self-contained (unlike `compile()`, which defaults to `false`).
